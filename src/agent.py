...
class AgentManager:
    ...
    def rotear_intencao(self, texto: str) -> bool:
        ...
        return bool(palavras & PALAVRAS_ACAO)
    ...
    async def chamar_groq_com_fallback(self, modelo_preferencial: str, kwargs: dict):
        ...
        for modelo in modelos:
            ...
            try:
                ...
                comp = await asyncio.to_thread(self.groq_client.chat.completions.create, **tentativa)
    ...
    async def processar_mensagem(self, message, mensagem_texto: str) -> str:
        ...
        try:
            for _ in range(limite_passos):
                ...
                comp = await self.chamar_groq_com_fallback(modelo_atual, kwargs)
            ...
            if not resposta_final:
                final = await self.chamar_groq_com_fallback(ConfigManager.MODELO_PEQUENO, {"messages": sessao})
                resposta_final = final.choices[0].message.content or ""
